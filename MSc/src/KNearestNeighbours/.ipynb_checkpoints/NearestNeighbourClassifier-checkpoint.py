...
class NearestNeighbourClassifier:
    ...
    def decide(self, U, x):
        ...
        # Posterior class probabilities from KNN 
        p = self.get_probabilities(x) 
        # shape: (n_classes,) # Expected utility for each action: EU[a] = sum_y P(y|x) * U[a, y] 
        expected_util = U @ p 
        # shape: (n_actions,) # Choose the action with the highest expected utility 
        return int(np.argmax(expected_util))
